...
这一章的主要内容包括硬件描述语言在不同领域中的应用，涵盖组合逻辑、时延、顺序逻辑等基础概念，同时介绍一些更为复杂的主题，如组合逻辑中使用的 `always` 语句、阻塞与非阻塞赋值的区别，以及有限状态机的设计等。这些主题的核心在于帮助设计者深入理解如何用 HDL 来描述数字电路的行为，并借助参数化模块和测试平台（testbenches）来优化设计。

本章探讨的主要主题如下：
- **Introduction**：介绍硬件描述语言的基本概念和重要性。
- **Combinational Logic**：探讨组合逻辑电路的设计与实现。
- **Delays**：时延在电路仿真中的影响及其建模方式。
- **Sequential Logic**：顺序逻辑电路的工作原理和设计方法。
...
右侧图表展示了计算机系统层次结构，从物理学原理到应用软件层面，说明了硬件描述语言主要作用于“数字电路”和“逻辑”这两个层面，作为连接微架构和底层模拟硬件的重要桥梁。
...
当前主流的硬件描述语言有两种：**SystemVerilog** 和 **VHDL**。

1. **SystemVerilog**：
   - 由 Gateway Design Automation 于 1984 年开发。
   - 1995 年成为 IEEE 标准（1364）。
   - 2005 年进行了扩展，形成了 IEEE 标准 1800-2009。
   - SystemVerilog 是 Verilog 语言的扩展版，增强了高级硬件描述的能力，特别是在验证和测试领域。
   
2. **VHDL 2008**：
   - 由美国国防部于 1981 年开发。
   - 1987 年成为 IEEE 标准（1076）。
   - 2008 年进行了最新修订（IEEE STD 1076-2008）。
   - VHDL 强调对硬件设计的高层次抽象，广泛用于航空航天、军事等高可靠性领域。
...
右侧的示意图展示了综合后的电路，其中 `y1` 到 `y5` 各自通过与门、或门、异或门等组合实现。对于每一对输入 `a` 和 `b` 的位线，电路将根据相应的按位运算符生成特定的输出逻辑。
...
顺序逻辑是硬件设计中的一种关键概念，它与组合逻辑不同，顺序逻辑的输出不仅取决于当前的输入，还依赖于之前的状态。顺序逻辑广泛应用于存储元件如锁存器（latches）、触发器（flip-flops）以及有限状态机（FSMs）的设计中。在 SystemVerilog 中，使用特定的编程习惯（idioms）来正确地描述这些顺序逻辑电路，确保在仿真与实际硬件之间保持一致性。

### 关键点：
- SystemVerilog 使用特定的编程风格（idioms）来描述锁存器、触发器和有限状态机等顺序逻辑。
- 其他编码风格可能在仿真时看似正确，但生成的硬件可能会出错。因此，严格遵循规范的顺序逻辑描述方法非常重要。

## `always` Statement

顺序逻辑通常通过 `always` 语句来实现，`always` 语句是 Verilog 和 SystemVerilog 中最常用的结构之一，它根据**敏感列表**（sensitivity list）中的信号变化来执行语句块。`always` 块的作用是描述需要在特定条件下触发的逻辑行为，尤其在时序逻辑中，比如基于时钟的触发器设计。
...
### 应用：
- **时钟触发的顺序逻辑**：常见于触发器设计，保证在时钟的上升沿或下降沿进行状态更新。
...
D 触发器（D Flip-Flop）是顺序逻辑电路中最常见的存储单元之一，用于在时钟信号的作用下存储和传递数据。每当时钟信号上升沿时，D 触发器将输入的数据 `d` 传递给输出 `q`，从而将数据锁存。
...
在顺序逻辑设计中，`always` 语句的结构对于触发器（Flip-Flop）和锁存器（Latch）的实现至关重要。
...
在 SystemVerilog 中，赋值语句分为**阻塞赋值（blocking assignment）**和**非阻塞赋值（nonblocking assignment）**两种。了解这两者的区别对于确保时序逻辑的正确性至关重要，尤其是在描述顺序逻辑电路时。

### Blocking vs. Nonblocking Assignment

1. **非阻塞赋值 (`<=`)**：
   - 非阻塞赋值同时执行，不依赖赋值顺序。
   - 通常用于**顺序逻辑**，例如触发器（flip-flop）中，用于在时钟边沿进行数据的并行更新。
...
通过遵循这些规则，设计者可以确保信号在顺序逻辑和组合逻辑中的行为正确，并避免潜在的时序问题。
...
此 FSM 使用一个状态寄存器和状态转移逻辑来实现三状态的循环行为。输出信号 `q` 仅在 FSM 处于 `S0` 状态时有效，其余状态输出为低。这种状态机模型在设计复杂的控制逻辑时非常有用，能够有效地描述系统的时序和行为。
...
这一例子展示了一个**Moore型有限状态机（FSM）**的实现，用于检测一个特定的输入序列。Moore FSM 的输出仅由当前状态决定，而不依赖于输入信号。这种状态机特别适合用于序列检测，例如检测连续的 "101" 输入模式。

### 状态机说明

这个 Moore FSM 由三个状态组成：
1. **S0**：初始状态，也为复位状态。当输入为 `1` 时，转移到 `S1`，否则保持在 `S0`。
2. **S1**：表示序列的第一个 `1` 已被检测到。输入为 `0` 时，转移到 `S2`；否则返回 `S0`。
3. **S2**：表示已检测到 "10" 序列。若输入为 `1`，则表示已检测到 "101" 序列，此时输出 `smile` 为高电平（`1`），然后回到初始状态 `S0`。

### 状态转换图

- **S0**（初始状态）：当输入为 `1` 时，进入 `S1` 状态，表示序列开始；输入为 `0` 时保持在 `S0`。
- **S1**（检测到 1）：当输入为 `0` 时，进入 `S2`，表示检测到 "10"；若输入为 `1`，则返回 `S0`。
- **S2**（检测到 10）：当输入为 `1` 时，表示已检测到 "101" 序列，进入复位状态 `S0`，并产生输出 `smile = 1`。

### Sequence Detector FSM in SystemVerilog

下面的 SystemVerilog 代码实现了这个 Moore 序列检测器：
...
module seqDetectMoore(input logic clk, reset, a,
  ...
  always_comb
    case (state)
      S0: if (a) nextstate = S1; else nextstate = S0;  // 检测到 1 后进入 S1
      S1: if (a) nextstate = S0; else nextstate = S2;  // 检测到 0 后进入 S2
      S2: if (a) nextstate = S0; else nextstate = S1;  // 完整序列 101 后返回 S0
      default: nextstate = S0;
    endcase

  // 输出逻辑：仅当状态为 S2 时 smile = 1
...
1. **状态寄存器**：
   - 使用 `always_ff` 块来实现同步逻辑。每当时钟上升沿到来时，如果 `reset` 为高，则状态重置为 `S0`；否则，状态更新为 `nextstate`。
   
2. **下一个状态逻辑**：
   - 通过 `always_comb` 来描述状态的转换。根据当前状态和输入 `a` 的值，FSM 会在 `S0`, `S1`, `S2` 之间切换。
   
3. **输出逻辑**：
   - 输出 `smile` 仅在状态为 `S2` 时为高电平，这意味着已经检测到 "101" 序列。

### 综合结果

通过状态寄存器、下一个状态逻辑和输出逻辑，这个 FSM 可以检测输入序列中的 "101" 模式，并在检测到该序列时输出一个高电平信号 `smile`。这是一个典型的 Moore 状态机示例，适用于序列检测等场景。
...
这一例展示了一个**Mealy 型有限状态机（FSM）**的实现，用于序列检测。与 Moore FSM 不同，Mealy FSM 的输出不仅取决于当前状态，还与输入信号密切相关。这样可以在状态变化的瞬间对输入做出响应，而不必等到下一个时钟周期。

### 状态转换图（Mealy FSM）

这个状态机有两个状态：
1. **S0**：初始状态，当输入为 `1` 时，转移到 `S1`，并且此时输出 `smile` 为 `0`。
2. **S1**：第二个状态，当输入为 `1` 时，输出 `smile` 变为 `1`，表示检测到了正确的输入序列。然后状态转移回 `S0`。

每个状态下的转移和输出由输入 `a` 决定：
- 在 `S0` 状态，若 `a = 1`，则转移到 `S1`，否则保持在 `S0`，并且输出 `smile = 0`。
- 在 `S1` 状态，若 `a = 1`，则输出 `smile = 1` 并返回 `S0`。

### Sequence Detector FSM in SystemVerilog (Mealy)

下列代码展示了如何用 SystemVerilog 实现这个 Mealy 序列检测器。
...
module seqDetectMealy(input logic clk, reset, a,
  ...
  // 下一个状态逻辑和输出逻辑
  always_comb begin
    smile = 1'b0;  // 默认情况下，输出为 0
    case (state)
      S0: if (a) nextstate = S1; else nextstate = S0;  // 检测到 1 后进入 S1
      S1: if (a) begin
             nextstate = S0;  // 检测到 1 后回到 S0
             smile = 1'b1;    // 输出 smile = 1
           end
           else nextstate = S1;
      default: nextstate = S0;
    endcase
...
1. **状态寄存器**：
   - 使用 `always_ff` 来描述状态寄存器的行为，每当时钟上升沿到来时，状态更新为下一个状态。如果 `reset` 信号有效，状态重置为 `S0`。
   
2. **下一个状态逻辑和输出逻辑**：
   - 使用 `always_comb` 描述状态转移和输出逻辑。根据当前状态和输入 `a`，状态机会在 `S0` 和 `S1` 之间切换。
   - 在 `S1` 状态时，如果输入 `a = 1`，输出 `smile` 立即变为 `1`，并返回 `S0` 状态。由于这是 Mealy 机器，因此输出的变化与输入信号的变化是同步的，无需等待下一个时钟周期。

### 综合结果

Mealy 状态机的输出在输入变化时立即更新。这种设计通常更为灵活，可以减少时钟周期延迟，但需要更复杂的设计来确保输入信号的稳定性。在这个例子中，FSM 用于检测输入 `a` 的连续变化，当输入序列符合条件时，立即产生 `smile` 信号。

与 Moore FSM 相比，Mealy FSM 在状态切换时更加敏感，因为它不仅根据状态，还会根据输入来决定输出。这使得它在某些需要快速响应的场景下更加高效。
